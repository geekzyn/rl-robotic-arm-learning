...
if __name__ == '__main__':
    ...
    hidden_dim = 512
    model_path =  join(dir, '../models/sac_multi')
    ...
    if args.test:
        ...
        env = GraspEnv(headless=False) # for visualizing in test
        trained_model_path1 = join(dir, '../models/trained_model/augmented_dense_reward/sac_multi')  # pre-trained model with augmented dense reward
        trained_model_path2 = join(dir, '../models/trained_model/dense_reward/sac_multi')  # pre-trained model with dense reward
        sac_trainer.load_model(trained_model_path1)  # new model after training
        for eps in range(30):
            state =  env.reset()
            episode_reward = 0
